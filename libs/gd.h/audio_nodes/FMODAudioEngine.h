...
namespace gd {
	...
	public:
		...
		std::string m_sFilePath;//0x4
		float m_fBackgroundMusicVolume;//0x8
		float m_fEffectsVolume;//0xC
		float m_fPulse1;//0x10
		float m_fPulse2;//0x14
		float m_fPulse3;//0x18
		int m_nPulseCounter;//0x1c
		bool m_bMetering;//0x1D
		bool m_bFading;//0x1E
		bool m_bFadeIn;//0x1F
		float m_fFadeInDuration;//0x23
		FMOD::System* m_pSystem;//0x23
		FMOD::Sound* m_pSound;
	...
	public:
		...
		void playEffect(std::string const& path, bool idk0, bool idk1) {
			reinterpret_cast<void(__thiscall*)(FMODAudioEngine*, bool, bool, std::string)>(
				base + 0x24160
				)(this, idk0, idk1, path);
		}
